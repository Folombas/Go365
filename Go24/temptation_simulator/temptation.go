...
type Temptation struct {
	...
	Category    string // Категория искушения
}

type Motivation struct {
	Text   string
	Effect string // "focus+", "willpower+", "knowledge+"
	XPBonus int   // Бонус опыта
}

var temptations = []Temptation{
	// Цифровые искушения
	{"CapCut Видеомонтаж", 85, "Установить программу и монтировать видео отпуска", 50, "digital"},
	{"Видеоигры", 70, "Поиграть в новую RPG-игру", 40, "digital"},
	{"Соцсети", 60, "Прокрутить ленту Instagram 2 часа", 30, "digital"},
	{"YouTube", 65, "Посмотреть 'ещё одно' видео про Go", 35, "digital"},
	{"Telegram каналы", 55, "Читать всё подряд вместо учёбы", 25, "digital"},
	{"Мобильные игры", 60, "Проверить ежедневные награды в играх", 30, "digital"},
	...
	{"Покупки онлайн", 55, "Купить ненужные вещи на маркетплейсе", 30, "shopping"},
	{"Новое железо", 70, "Заказать новую клавиатуру/мышь", 40, "shopping"},
	{"Курсы 'всё в одном'", 60, "Купить ещё один курс вместо практики", 35, "shopping"},
	
	// Еда и здоровье
	{"Фастфуд", 50, "Съесть пиццу вместо здорового ужина", 20, "health"},
	{"Сладкое", 40, "Съесть шоколадку за шоколадкой", 15, "health"},
	...
	// Особо опасные (боссы)
	{"n8n Ботомания", 90, "Создать ещё одного бота вместо изучения Go", 60, "boss"},
	{"Blender Прокрастинация", 85, "3D-моделирование 'для расслабления'", 55, "boss"},
	{"Депрессия", 95, "Лечь спать и не вставать 3 дня", 70, "boss"},
	{"Выгорание", 88, "Бросить всё и уехать в горы", 65, "boss"},
}
...
// Босс-искушения (особо опасные)
var bossTemptations = []Temptation{
	{"👹 CAPCUT МОНТЁР", 95, "Установи CapCut и монтируй 8 часов подряд!", 80, "boss"},
	{"👹 ИГРОВОЙ ЗАВИСИМОН", 92, "Новая AAA-игра, просто 'один квест'!", 75, "boss"},
	{"👹 СОЦСЕТЕЙ ДЕМОНИУС", 88, "Листай ленту до 3 часов ночи!", 65, "boss"},
	{"👹 АЛКОГОЛЬНЫЙ ПРИЗРАК", 90, "Выпей 'для расслабления' после работы!", 70, "boss"},
...
// GetRandomTemptationByCategory возвращает случайное искушение из категории
func GetRandomTemptationByCategory(category string) Temptation {
	categoryTemptations := GetTemptationsByCategory(category)
	if len(categoryTemptations) == 0 {
		return GenerateTemptation()
	}
	index := rand.Intn(len(categoryTemptations))
	return categoryTemptations[index]
}
